    ...
    if ($id!=0) {
        try {
            ...
            if ($connection->connect_errno!=0) {
				...
				$_SESSION['reservation_name'] = $row['name'];
                $_SESSION['reservation_last_name'] = $row['last_name'];
                $_SESSION['reservation_class'] = $row['class'];
				$_SESSION['reservation_phone_number'] = $row['phone_number'];
				$_SESSION['reservation_email'] = $row['email'];
				$_SESSION['reservation_id_book'] = $row['id_book'];

				$id_book = $_SESSION['reservation_id_book'];
				$sql = "SELECT * FROM book WHERE id='$id_book'";
                $result = $connection->query($sql);
                $row = $result->fetch_assoc();
	...
	<body class="is-preload">
			<div id="wrapper">
					...
					<div id="main">
						<div class="inner">
							<header>
								<h1>Sprawdź dane użytkownika</h1>
								<section id="search">
									<h2>Dane czytelnika:</h2>
									<p>Imię: <strong><?php echo ''.$_SESSION['reservation_name'].'' ?></strong></p>
									<p>Nazwisko: <strong><?php echo ''.$_SESSION['reservation_last_name'].'' ?></strong></p>
									<p>Klasa: <strong><?php echo ''.$_SESSION['reservation_class'].'' ?></strong></p>
									<p>Numer telefonu: <strong><?php echo ''.$_SESSION['reservation_phone_number'].'' ?></strong></p>
									<p>Email: <strong><?php echo ''.$_SESSION['reservation_email'].'' ?></strong></p>
									...
									<ul class="actions">
										<li><a href="add/" class="button primary icon solid fa-save">Dalej</a></li>
										<li><a href="del.php/?id=<?php echo $_GET['id'] ?>" class="button icon solid fa-trash">Anuluj</a></li>
									</ul>
								</section>
							</header>
                        </div>
