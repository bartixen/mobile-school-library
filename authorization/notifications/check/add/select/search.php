        ...
        if ($connection->connect_errno != 0) {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    $buttonClass = 'button icon solid fa-sign-in-alt';
                    $linkURL = 'summary/?id=' . $row["id"];
                    if ($row["status"] === 'Wypożyczona') {
                        $buttonClass .= ' disabled';
                        $linkURL = 'javascript:void(0);';
                    }
                    echo "<tr><th>" . $row["id"] . "</th><th>" . $row["title"] . "</th><th>" . $row["author"] . "</th><th>" . $row["record_number"] . "</th><th>" . $row["status"] . "</th><th><a href='$linkURL'><button type='button' class='$buttonClass'>Wybierz</button></a></th></tr>";
                }
                ...
                if ($result->num_rows > 0) {
                    echo "<div style='overflow-x: auto;'><table><thead><tr><th>ID</th><th>Tytuł</th><th>Autor</th><th>Numer ewidencji</th><th>Status</th><th></th></tr></thead><tbody>";
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr><th>" . $row["id"] . "</th><th>" . $row["title"] . "</th><th>" . $row["author"] . "</th><th>" . $row["record_number"] . "</th><th>" . $row["status"] . "</th><th><a href='summary/?id=" . $row["id"] . "'><button type='button' class='button icon solid fa-sign-in-alt'>Wybierz</button></a></th></tr>";
                    }
                    echo "</tbody></table></div>";
                } else {
